...
function AkrabV3Page() {
  ...
  useEffect(() => {
    fetch('/api/v3/xl/stock-khfy', { method: 'POST' })
      .then((res) => res.json())
  ...
  const handleOrder = async (code) => {
    ...
    try {
      const res = await fetch('/api/xl/akrabv3/order', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ code, destination: number }) });
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold text-white mb-6">Akrab V3 (KHFY)</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {products.map((p, i) => (
          <div key={i} className="bg-darker rounded-xl p-4 border border-gray-700">
            <h4 className="text-white font-semibold mb-1">{p.nama || p.name}</h4>
            <p className="text-gray-400 text-sm mb-1">Kode: {p.code || p.type}</p>
            <p className="text-gray-400 text-sm mb-2">Stok: {p.stock ?? 'N/A'}</p>
            <p className="text-green-400 font-bold mb-3">Rp {(p.harga || 0).toLocaleString('id-ID')}</p>
            <button onClick={() => handleOrder(p.code || p.type)} disabled={loading} className="w-full bg-primary hover:bg-secondary text-white py-2 rounded-lg text-sm transition-colors disabled:opacity-50">Order</button>
          </div>
        ))}
